...
//? import service
import { updateCart } from "@/services/usersServices";
...
function Card({ data }) {
  ...
  const {
    data: cartUpdateData,
    isPending: cartUpdatePending,
    mutateAsync: mutateUpdateCart,
  } = useMutation({ mutationFn: updateCart });
  ...
  const orderHandler = async (data) => {
    if (user) {
      try {
        const { message } = await mutateUpdateCart(data);
    ...
    <div className="mt-12 max-w-xs">
      <div className="bg-white shadow-sm shadow-secondary-100/40 rounded-xl relative p-3 flex flex-col h-full hover:shadow-lg hover:shadow-secondary-100/40 transition-all duration-300 ease-in-out">
        ...
        <div>
          ...
          {isPending ? (
          ...
          ) : user?.cart.some((item) => item.id === data.id) ? (
            <button className="px-4 py-2 w-full text-blue-500 bg-blue-100/50 rounded-xl font-bold mt-2">
            ...
            <div className="flex items-center justify-between">
              <div>
                <button
                  onClick={() => orderHandler(data)}
                  className="px-4 py-2 rounded-2xl text-white font-bold bg-blue-600 hover:bg-blue-500 transition-all shadow-md shadow-blue-300 text-base"
                >
                  ثبت نام در دوره
                </button>
              </div>
